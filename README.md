<div align="center">
  <img src="https://raw.githubusercontent.com/taazkareem/google-lens-pro/main/assets/banner.jpg" alt="Google Lens Pro — Unofficial Python API, Visual Search & Multimodal Commercial Intelligence" width="100%" style="border-radius: 8px;">
</div>

<br/>

# Google Lens Pro (`google-lens-pro`)

[![PyPI version](https://img.shields.io/pypi/v/google-lens-pro?color=blue)](https://pypi.org/project/google-lens-pro/)
[![Python versions](https://img.shields.io/pypi/pyversions/google-lens-pro?color=blue)](https://pypi.org/project/google-lens-pro/)
...
- [Why Google Lens Pro? (Comparison)](#why-google-lens-pro)
...
- **Permanent Session Authentication**:
  - `lens login` command captures Google account credentials once to run 100% headless visual searches indefinitely.
  - Automatic session loading from `~/.config/google-lens-pro/session.json`.
...
- **Dual Synchronous & Asynchronous APIs**: `GoogleLens`, `LensScraper`, and `AsyncGoogleLens` with native concurrent destination enrichment (`enrich_async`) and fusion (`fuse_async`).
- **Command-Line Interface**: Fast `lens` and `google-lens` CLI for quick visual searches, shopping, OCR, and session management.
...
- **🛍️ E-Commerce & Multi-Store Pricing**: Discover where products, sneakers, clothing, or electronics are sold online and compare prices across 50+ merchants (StockX, GOAT, Flight Club, eBay, Walmart).
...
```
*(Provides the `lens` and `google-lens` CLI commands)*
...
6. **🏷️ Item Condition & SKU Extraction (`condition`, `sku`)**: Captures condition (`new`, `used`, `refurbished`) and product identifiers (GTIN, UPC, SKU) for resale pricing and catalog matching.
7. **🔗 Canonical URL Unwrapping**: Decodes opaque `google.com/url?q=...` redirects into direct, clean merchant product links, stripping UTM parameters, ad click-trackers, and Chrome text fragments.
8. **💰 Normalized Pricing & Best Deals**: Auto-extracts numeric prices and currencies (USD, EUR, GBP, INR, etc.), computing market average/range and highlighting the lowest-priced verified seller.
9. **🏪 Merchant Categorization**: Labels sellers as Official Brand stores (e.g. Nike, Apple), Major Marketplaces (Amazon, eBay, Walmart), or Resellers (StockX, GOAT, Chrono24).
10. **📁 Clean JSON-First Data Pipelines**: Direct `--export-json` (or `--export`) flag for developer-grade hierarchical data without sparse, inconsistent CSV columns or `null` noise.
11. **⚡ Multi-Device Activations**: Authorize up to 3 machines simultaneously via CLI (`lens pro activate <key>`) or purchase directly (`lens buy pro`) with self-service license management in the Polar customer portal.
12. **🎯 Category-Agnostic Semantic Match Evaluation (`relevance`)**: Separates exact product matches from accessories, lookalikes, and unrelated SERP noise (`exact_match`, `similar`, `reference`, `unrelated`) with transparent, human-readable rationale (`relevance_reason`).
13. **🏷️ Target Product Identification (`target_product`)**: Automatically detects the canonical item name and silhouette from visual cues and metadata, anchoring all price comparison and match score analytics.

---

## Why Google Lens Pro?

| Feature | `google-lens-pro` (Free / MIT) | `google-lens-pro` (Pro) | SerpApi / Proxy APIs | Google Cloud Vision API |
| :--- | :--- | :--- | :--- | :--- |
...
lens login --env
```
A browser window will open. Sign into your Google account (or dismiss any prompts) and press **[Enter]** in your terminal. Your session will be saved securely to `~/.config/google-lens-pro/session.json` (and written to `.env` if `--env` is passed).
...
> **Note:** If a `.env` file is present in the working directory, `google-lens-pro` will automatically load any `LENS_*` variables defined in it.
...
```python
from google_lens_pro import GoogleLens, LensScraper
...
```python
import asyncio
from google_lens_pro import AsyncGoogleLens, AsyncLensScraper
